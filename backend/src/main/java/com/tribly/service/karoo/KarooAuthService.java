...
/**
 * Service handling device code OAuth flow (RFC 8628) for Karoo devices.
 *
 * <p>Flow:
 *
 * <ol>
 *   <li>Device calls POST /api/karoo/oauth/device → gets device_code + user_code
 *   <li>Device displays user_code and QR for verification_uri_complete
 *   <li>User scans QR → opens /karoo/verify?code=ABC123 on phone
 *   <li>User authenticates via OTP email on frontend
 *   <li>Frontend calls POST /api/karoo/oauth/complete to mark code as authorized
 *   <li>Device polls POST /api/karoo/oauth/token until authorized
 *   <li>Device receives access + refresh tokens
 * </ol>
 */
@ApplicationScoped
public class KarooAuthService {

  private static final Logger LOG = Logger.getLogger(KarooAuthService.class);
  ...
  @Inject KarooJwtService karooJwtService;
   ...
   */
  @Public
  public KarooDeviceCodeResponse initiateDeviceCodeFlow() {
    ...
    String verificationUri = baseUrl + "/karoo/verify";
    ...
    LOG.infov("Initiated device code flow, user_code={0}", userCode);

    return KarooDeviceCodeResponse.builder()
  ...
  public void completeDeviceCodeFlow(String userCode, Long userId, Long domainId) {
    ...
    deviceCodes.put(
        deviceCode,
        new DeviceCodeData(
            codeData.userCode(), codeData.domainId(), codeData.expiresAt(), userId, true));

    LOG.infov("Device code authorized for user {0}, user_code={1}", userId, userCode);
  }
  ...
  @Transactional
  @Public
  public KarooTokenResponse exchangeDeviceCode(String deviceCode) {
    DeviceCodeData codeData = deviceCodes.get(deviceCode);
  ...
  @Transactional
  @Public
  public KarooTokenResponse refreshToken(String refreshToken) {
    ...
    String accessToken = karooJwtService.generateAccessToken(user);

    return KarooTokenResponse.builder()
        .accessToken(accessToken)
        .tokenType("Bearer")
        .expiresIn(karooJwtService.getAccessTokenExpirySeconds())
        .refreshToken(null)
        .build();
  }
  ...
  private KarooTokenResponse createTokenResponse(User user) {
    // Generate tokens
    String accessToken = karooJwtService.generateAccessToken(user);
    String refreshToken = generateSecureToken();
    String refreshTokenHash = hashToken(refreshToken);

    // Create session with Karoo-specific expiry
    AuthSession session =
        new AuthSession(
            user,
            refreshTokenHash,
            Instant.now().plus(Duration.ofDays(karooJwtService.getRefreshTokenExpiryDays())));
    session.setUserAgent("Karoo Device");
