...
public class IcsGenerationService {
  ...
  private void appendEvent(StringBuilder ics, CalendarEventDto event) {
    ...
    // Categories
    if (event.type() == CalendarEventType.RIDE) {
      ics.append("CATEGORIES:RIDE\r\n");
    } else if (event.type() == CalendarEventType.TRIP_STAGE) {
      ics.append("CATEGORIES:TRIP\r\n");
    }
  ...
  private String buildEventUrl(CalendarEventDto event) {
    if (event.type() == CalendarEventType.RIDE) {
      return frontendUrl + "/teams/" + event.teamSlug() + "/rides/" + event.entitySlug();
    } else if (event.type() == CalendarEventType.TRIP_STAGE && event.tripSlug() != null) {
      return frontendUrl
          + "/teams/"
          + event.teamSlug()
          + "/trips/"
