...
public class FileTypeDetector {
   ...
   *
   * <p>Acceptance has two stages: first the Magika label must be in the category's allowlist; if
   * not, a filename-extension fallback is consulted (see {@link #matchesByExtension}) which covers
   * small/empty GPX and FIT files where Magika cannot reach a confident decision. When the
   * extension fallback is the deciding factor a WARN is logged so a silent Magika regression is
   * still visible.
   *
   * <p>Throws {@link BadRequestException} with {@link ErrorCode#FILE_TYPE_REJECTED} when neither
   * the allowlist nor the extension fallback accepts the file, or {@link
  ...
  public DetectedFileType detectAndValidate(
    ...
    if (!acceptedByLabel) {
      if (!matchesByExtension(category, fileName)) {
        LOG.warnf(
            "Rejecting upload fileName=%s assetType=%s detectedLabel=%s category=%s allowed=%s",
            fileName, assetType, detected.label(), category, category.getAllowedLabels());
        throw new BadRequestException(ErrorCode.FILE_TYPE_REJECTED);
      }
      LOG.warnf(
          "Accepting upload via extension fallback (Magika label outside allowlist):"
              + " fileName=%s assetType=%s detectedLabel=%s confidence=%.3f category=%s allowed=%s",
          fileName,
          assetType,
          detected.label(),
          detected.confidence(),
          category,
          category.getAllowedLabels());
    }
    return detected;
  }
