...
- **`hillshade-shadow-color` / `hillshade-highlight-color` sont désactivées sur iOS** — le SDK MapLibre iOS traite ces deux propriétés `colorArray` sans vérifier la classe de la constante et **abandonne le process** (`-[UIDeviceRGBColor count]`, `NSInvalidArgumentException`) ; ce n'est pas une exception Dart, aucun `try/catch` ne la rattrape. `_hillshadeColorsCrashOnIos` (`lib/core/pdl/map/pdl_map_controller.dart`) omet donc les deux teintes sur iOS, qui garde l'ombrage aux couleurs par défaut. **Vérifier à chaque montée de version de `maplibre`** (0.3.6 aujourd'hui) si [josxha/flutter-maplibre#571](https://github.com/josxha/flutter-maplibre/pull/571) est fusionné et publié : si oui, remonter la dépendance, supprimer le drapeau et cette ligne
...
- **The bell is back, and it reads a real API** — `features/notifications/` serves the home bell, the `/notifications` inbox and the profile's preference matrix off `/api/notifications*`. What has not changed is the rule that put it away in v2: **no control without effect**. So the preference *matrix* is not rendered when `NotificationPreferencesDto.channels` comes back empty, which is the default (`IN_APP` is never configurable, and `PUSH` only shows up once the server has an FCM service account); the per-team mute switches still are, since muting a team also empties the inbox of its announcements — the section disappears, header included, only with neither channel nor team. The daily-digest switch shows only when `EMAIL` is a channel. Two other invariants hold there: the API carries **no rendered text**, so the wording, the icon and the destination are the client's (`presentation/notification_display.dart`), and the **destination comes from `subjectType`, never from `type`** — an unknown subject renders a line that simply isn't tappable, rather than a `go` to a route that doesn't exist. The badge polls `unread-count` at most once a minute (`kUnreadPollInterval`) and stops with the session
- **Push is opt-in from the inbox, and the app never asks at launch** — `PushController`
  (`features/notifications/providers/push_provider.dart`) registers a token *only* once the member
  grants permission, and `PushActivationBanner` only offers to ask when **`PUSH` is in
  `NotificationPreferencesDto.channels`**, i.e. when the server can actually send. The system
  dialog appears once in the life of an installation: spending it at launch, before the member has
  seen what the app notifies about, spends it badly. Two more rules hold there: a tapped
  notification opens `data.path` **as the server computed it** (never a route rebuilt from the
  subject) and goes through the *same* `pendingPushRouteProvider` → `main.dart` waiting logic as a
  web link, so the back stack is rebuilt; and nothing above `PushGateway` touches Firebase, which
  is what lets `push_test.dart` run at all
- **Attachments render through `core/widgets/media_attachments.dart`, everywhere** — the eight screens carrying a `MediaDto` (ride, post, ad, route, trip, stage, team page, about) share one `MediaAttachments`. The three hand-rolled copies it replaced had already drifted: the ride's listed the files with **no button**, naming an attachment while giving no way to open it. Attachments only arrive in the full view — a `COMPACT` list doesn't carry them, which is why no card shows files
