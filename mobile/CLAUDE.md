...
- **The bell is back, and it reads a real API** — `features/notifications/` serves the home bell, the `/notifications` inbox and the profile's preference matrix off `/api/notifications*`. What has not changed is the rule that put it away in v2: **no control without effect**. So the preference section is *not rendered at all* — its own header included — when `NotificationPreferencesDto.channels` comes back empty, which is the default (`IN_APP` is never configurable, and `PUSH` only shows up once the server has an FCM service account). Two other invariants hold there: the API carries **no rendered text**, so the wording, the icon and the destination are the client's (`presentation/notification_display.dart`), and the **destination comes from `subjectType`, never from `type`** — an unknown subject renders a line that simply isn't tappable, rather than a `go` to a route that doesn't exist. The badge polls `unread-count` at most once a minute (`kUnreadPollInterval`) and stops with the session
