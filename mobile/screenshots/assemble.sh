...
#
#   1. ./screenshots/capture.sh                      -> screenshots/flat/<appareil>/<locale>/NN-*.png
#   2. kou generate screenshots/koubou/<appareil>.yaml
#                                                    -> screenshots/koubou/out/<appareil>/<locale>/<cadre>/NN-*.png
#   3. ./screenshots/assemble.sh                     -> screenshots/appstore/<type>/<locale>/NN-*.png (Git LFS)
#
...
for device in $devices; do
  ...
  bold "$device -> screenshots/appstore/$type : $(echo "$locales" | tr '\n' ' ')"
  for loc in $locales; do
    ...
    dst="screenshots/appstore/$type/$loc"
    mkdir -p "$dst"
...
for kind, expect in SETS.items():
    root = os.path.join("screenshots", "appstore", kind)
...
if [ "$run_validate" -eq 1 ]; then
  if command -v asc >/dev/null 2>&1; then
    for device in $devices; do
      type=$(display_type_for "$device")
      for dir in screenshots/appstore/"$type"/*/; do
...
bold "OK — screenshots/appstore/IPHONE_65/ et IPAD_PRO_3GEN_129/ sont prêts à l'envoi"
