...
/// Type
 ...
 bool get full;/// Plain-text opening of the markdown body, flattened (links become their label) and cut on a word boundary at about 200 characters. Null when the body holds no text. Lets a list row render its two lines without the body being sent at all — see the 'view' parameter.
 ...
 String? get registeredGroupId;/// Number of comments, replies included. Absent when the caller may not read the comments of this ride — comments are members-only, so an outsider is told nothing, not even zero.
...
int get hashCode {
  final _this = this as RideDto;
  return Object.hashAll([runtimeType,_this.type,_this.team,_this.id,_this.slug,_this.name,_this.media,_this.dateTime,_this.status,_this.visibility,_this.participantCount,_this.groupCount,const DeepCollectionEquality().hash(_this.groups),const DeepCollectionEquality().hash(_this.topParticipants),_this.deleted,_this.registered,_this.full,_this.excerpt,_this.publishAt,_this.createdAt,_this.routeSlug,_this.startPlace,_this.endPlace,_this.thumbnailLightUrl,_this.thumbnailDarkUrl,_this.thumbnailUrl,_this.registeredGroupId,_this.commentCount]);
}

@override
String toString() {
  final _this = this as RideDto;
  return 'RideDto(type: ${_this.type}, team: ${_this.team}, id: ${_this.id}, slug: ${_this.slug}, name: ${_this.name}, media: ${_this.media}, dateTime: ${_this.dateTime}, status: ${_this.status}, visibility: ${_this.visibility}, participantCount: ${_this.participantCount}, groupCount: ${_this.groupCount}, groups: ${_this.groups}, topParticipants: ${_this.topParticipants}, deleted: ${_this.deleted}, registered: ${_this.registered}, full: ${_this.full}, excerpt: ${_this.excerpt}, publishAt: ${_this.publishAt}, createdAt: ${_this.createdAt}, routeSlug: ${_this.routeSlug}, startPlace: ${_this.startPlace}, endPlace: ${_this.endPlace}, thumbnailLightUrl: ${_this.thumbnailLightUrl}, thumbnailDarkUrl: ${_this.thumbnailDarkUrl}, thumbnailUrl: ${_this.thumbnailUrl}, registeredGroupId: ${_this.registeredGroupId}, commentCount: ${_this.commentCount})';
}
...
@optionalTypeArgs TResult when<TResult extends Object?>(TResult Function( String type,  TeamPublicationDto team,  String id,  String slug,  String name,  MediaDto media,  String dateTime,  String status,  String visibility,  int participantCount,  int groupCount,  List<RideGroupDto> groups,  List<PublicUserDto> topParticipants,  bool deleted,  bool registered,  bool full,  String? excerpt,  String? publishAt,  String? createdAt,  String? routeSlug,  PlaceDetailDto? startPlace,  PlaceDetailDto? endPlace,  String? thumbnailLightUrl,  String? thumbnailDarkUrl,  String? thumbnailUrl,  String? registeredGroupId,  int? commentCount)  $default,) {final _that = this;
switch (_that) {
case _RideDto():
return $default(_that.type,_that.team,_that.id,_that.slug,_that.name,_that.media,_that.dateTime,_that.status,_that.visibility,_that.participantCount,_that.groupCount,_that.groups,_that.topParticipants,_that.deleted,_that.registered,_that.full,_that.excerpt,_that.publishAt,_that.createdAt,_that.routeSlug,_that.startPlace,_that.endPlace,_that.thumbnailLightUrl,_that.thumbnailDarkUrl,_that.thumbnailUrl,_that.registeredGroupId,_that.commentCount);case _:
  throw StateError('Unexpected subclass');
...
class _RideDto implements RideDto {
  const _RideDto({required this.type, required this.team, required this.id, required this.slug, required this.name, required this.media, required this.dateTime, required this.status, required this.visibility, required this.participantCount, required this.groupCount, required  List<RideGroupDto> groups, required  List<PublicUserDto> topParticipants, required this.deleted, required this.registered, required this.full, this.excerpt, this.publishAt, this.createdAt, this.routeSlug, this.startPlace, this.endPlace, this.thumbnailLightUrl, this.thumbnailDarkUrl, this.thumbnailUrl, this.registeredGroupId, this.commentCount}): _groups = groups,_topParticipants = topParticipants;
...
@override
int get hashCode {
    return Object.hashAll([runtimeType,type,team,id,slug,name,media,dateTime,status,visibility,participantCount,groupCount,const DeepCollectionEquality().hash(_groups),const DeepCollectionEquality().hash(_topParticipants),deleted,registered,full,excerpt,publishAt,createdAt,routeSlug,startPlace,endPlace,thumbnailLightUrl,thumbnailDarkUrl,thumbnailUrl,registeredGroupId,commentCount]);
}
