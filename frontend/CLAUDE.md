...
- **The notification UI reads a real API — never mock it** — the header bell (`components/notification/NotificationBell.tsx`), the `/notifications` page and the profile's preference matrix all come from `/api/notifications*`. Two rules hold there: **the API carries no rendered text**, so wording, icon and destination are the client's (`components/notification/notificationDisplay.ts`, keyed on `type` and `subjectType`); and **only the channels `NotificationPreferencesDto.channels` lists get a column** — `IN_APP` is never configurable and `PUSH` is only listed when the server has an FCM account, so an empty list hides the matrix (and the digest switch, which needs `EMAIL`) — but the per-team mute switches (`teams`) still show, since muting also keeps a team's announcements out of the inbox. The team webhook (`components/team/TeamWebhookSettings.tsx`) never shows more of its URL than `maskedUrl`: the URL is a secret. See §12 of `../docs/plans/2026-09-18-notifications.md`.
