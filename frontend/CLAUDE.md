...
- **The notification UI reads a real API — never mock it** — the header bell (`components/notification/NotificationBell.tsx`), the `/notifications` page and the profile's preference matrix all come from `/api/notifications*`. Two rules hold there: **the API carries no rendered text**, so wording, icon and destination are the client's (`components/notification/notificationDisplay.ts`, keyed on `type` and `subjectType`); and **only the channels `NotificationPreferencesDto.channels` lists get a column** — `IN_APP` is never configurable and `PUSH` has no sender yet, so an empty list hides the whole section instead of rendering an empty matrix. `…/devices` (push registration) still doesn't exist: that's phase 4 of `../docs/plans/2026-09-18-notifications.md`.
