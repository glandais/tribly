 ...
 *
 * `/routes/bulk` (`getGetRoutesBulkQueryKey`) is keyed on its own params (slug set, simplify,
 * points…) — a query key disjoint from the single-route key's prefix, so react-query's partial
